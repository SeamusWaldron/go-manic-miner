...
// ZX Spectrum ROM font data for printable ASCII characters 32-127.
// Each character is 8 bytes (8x8 pixels).
var zxFont = [96][8]byte{
	...
	{0x00, 0x7F, 0x08, 0x08, 0x08, 0x08, 0x08, 0x00}, // 84 'T'
	...
	{0x00, 0x22, 0x14, 0x08, 0x08, 0x08, 0x08, 0x00}, // 89 'Y'
...
func PrintMessage(target *ebiten.Image, x, y int, msg string, attr byte) {
	...
	for i, ch := range msg {
		...
		printChar(target, charX, y, byte(ch), ink, paper)
	}
}

func printChar(target *ebiten.Image, x, y int, ch byte, ink, paper color.RGBA) {
	idx := int(ch) - 32
	if idx < 0 || idx >= len(zxFont) {
		idx = 0
	}
	bitmap := zxFont[idx]
	...
	for row := 0; row < 8; row++ {
		...
		for bit := 7; bit >= 0; bit-- {
			...
			if b&(1<<uint(bit)) != 0 {
				target.Set(px, py, ink)
			} else {
				target.Set(px, py, paper)
			}
		}
	}
}
